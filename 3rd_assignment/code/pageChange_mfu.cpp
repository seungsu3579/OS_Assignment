...
int pageChange_mfu(Task *&currentCpuTask, PageBundle **physicalMem, int physicalNum, int aid) {

    PageBundle* deleteBundle = nullptr;
    int accessCountMin = -100;
    int targetIndex;
    int returnAid = 0;
    ...
    for(int i = 0 ; i < physicalNum ; i++){
        if(physicalMem[i] != nullptr && physicalMem[i]->aid != -1){
            ...
            if(physicalMem[i]->accessCount > accessCountMin){
                deleteBundle = physicalMem[i];
                accessCountMin = physicalMem[i]->accessCount;
                targetIndex = i;
            }else if(physicalMem[i]->accessCount == accessCountMin){
